...
extension VectorTests.Unit {
    ...
    @Test
    func `init creates vector with correct bounds`() throws {
    ...
    @Test
    func `isEmpty returns true for empty vector`() throws {
    ...
    @Test
    func `isEmpty returns false for non-empty vector`() throws {
...
extension VectorTests.EdgeCase {
    ...
    @Test
    func `empty vector forEach does nothing`() throws {
    ...
    @Test
    func `empty vector satisfies.all returns true`() throws {
        var vector = try Vector(0..<0) { $0 }
        #expect(vector.satisfies.all { _ in false })
    }

    @Test
    func `empty vector satisfies.any returns false`() throws {
        var vector = try Vector(0..<0) { $0 }
        #expect(!vector.satisfies.any { _ in true })
    }

    @Test
    func `empty vector first returns nil`() throws {
        var vector = try Vector(0..<0) { $0 }
        #expect(vector.first { _ in true } == nil)
    }

    @Test
    func `empty vector count(where:) returns zero`() throws {
        let vector = try Vector(0..<0) { $0 }
        #expect(vector.count(where: { _ in true }) == 0)
    }

    @Test
    func `single element vector works correctly`() throws {
    ...
    @Test
    func `large vector count is efficient (O(1))`() throws {
...
extension VectorReversedTests.EdgeCase {
    ...
    @Test
    func `empty reversed vector works`() throws {
        let vector = try Vector(0..<0) { $0 }
        var reversed = vector.reversed()
        #expect(reversed.isEmpty)
...
extension VectorInvariantTests.Iterator {
    ...
    @Test
    func `INVARIANT: Iterator count matches vector.count exactly`() throws {
        for size in [0, 1, 2, 10, 100, 1000] {
            ...
            #expect(iteratedCount == vector.count,
                   "Size \(size): iterated \(iteratedCount) but count is \(vector.count)")
        }
    }

    @Test
    func `INVARIANT: Reversed iterator count matches vector.count exactly`() throws {
        for size in [0, 1, 2, 10, 100, 1000] {
            let vector = try Vector(0..<size) { $0 }
            var iterator = vector.reversed().makeIterator()
            var iteratedCount: Vector<Int>.Index.Count = 0
...
extension VectorInvariantTests.Consistency {
    ...
    func `INVARIANT: count(where: { true }) == count property`() throws {
        for size in [0, 1, 5, 100] {
            ...
            let bool = try countWhere == Vector<Int>.Index.Count(size)
            #expect(bool,
    ...
    @Test
    func `INVARIANT: reduce.into(initial) { } returns initial for empty vector`() throws {
        var vector = try Vector(0..<0) { $0 }
    ...
    @Test
    func `INVARIANT: reduce.from(initial) { } returns initial for empty vector`() throws {
        var vector = try Vector(0..<0) { $0 }
...
extension VectorInvariantTests.Drain {
    ...
    @Test
    func `INVARIANT: drain empties the vector completely`() throws {
        var vector = try Vector(0..<10) { $0 }
        var drained: [Int] = []
    ...
    @Test
    func `INVARIANT: drain on empty vector does nothing`() throws {
        var vector = try Vector(0..<0) { $0 }
        var drainCount = 0
    ...
    @Test
    func `INVARIANT: reversed drain empties the vector completely`() throws {
        let vector = try Vector(0..<10) { $0 }
        var reversed = vector.reversed()
        var drained: [Int] = []
...
extension VectorInvariantTests.Boundaries {
    ...
    @Test
    func `INVARIANT: Offset vectors work correctly`() throws {
    ...
    @Test
    func `INVARIANT: Large offset vectors work correctly`() throws {
        let start = 1_000_000
    ...
    @Test
    func `INVARIANT: Negative start vectors work`() throws {
        let vector = try Vector(-5..<5) { $0 }
        #expect(vector.count == 10)
...
extension VectorStressTests.Stress {
    ...
    @Test
    func `STRESS: Many small vectors maintain invariants`() throws {
        for i in 0..<100 {
...
extension VectorDropPrefixTests.Drop {
    ...
    @Test
    func `drop.first returns Vector with adjusted start (O(1))`() throws {
        let vector = try Vector(0..<10) { $0 }
    ...
    @Test
    func `drop.first with count >= size returns empty vector`() throws {
        let vector = try Vector(0..<5) { $0 }
    ...
    @Test
    func `drop.first(0) returns equivalent vector`() throws {
        let vector = try Vector(0..<5) { $0 }
...
extension VectorDropPrefixTests.Prefix {
    ...
    @Test
    func `prefix.first returns Vector with adjusted end (O(1))`() throws {
        let vector = try Vector(0..<10) { $0 }
    ...
    @Test
    func `prefix.first with count >= size returns equivalent vector`() throws {
        let vector = try Vector(0..<5) { $0 }
    ...
    @Test
    func `prefix.first(0) returns empty vector`() throws {
        let vector = try Vector(0..<5) { $0 }
...
extension VectorDropPrefixTests.Reversed {
    ...
    @Test
    func `reversed empty vector drop/prefix`() throws {
        let vector = try Vector(0..<0) { $0 }
        let reversed = vector.reversed()
...
extension VectorDropPrefixInvariantTests.Invariants {
    ...
    func `INVARIANT: drop(0) and prefix(count) are identity operations`() throws {
        for size in [0, 1, 5, 20] {
            ...
            let afterPrefixAll = vector.prefix.first(try Vector<Int>.Index.Count(size))
            #expect(afterPrefixAll.count == vector.count)

            // prefix.first(size + 100) should also be identity
            let afterPrefixMore = vector.prefix.first(try Vector<Int>.Index.Count(size + 100))
...
extension VectorCardinalDistanceTests.Invariants {
    ...
    func `INVARIANT: count equals cardinal distance between positions`(start: Vector<UInt>.Index, end: Vector<UInt>.Index) throws {
    ...
    func `INVARIANT: count matches iteration count exactly`(start: Vector<UInt>.Index, end: Vector<UInt>.Index) throws {
        let vector = try Vector(start: start, end: end)
    ...
    @Test
    func `INVARIANT: reversed vector preserves count`() throws {
        let vector: Vector = Vector(count: 100)
        let reversed = vector.reversed()
...
extension VectorCardinalDistanceTests.LargeVectors {
    ...
    @Test
    func `INVARIANT: vectors exceeding Int.max distance work`() throws {
    ...
    @Test
    func `INVARIANT: offset vectors with large distances work`() throws {
        ...
        let cardinalDistance = try vector.start.position.distance.forward(to: vector.end.position)
        #expect(vector.count == Vector<UInt>.Index.Count(cardinalDistance))
    }

    @Test
    func `INVARIANT: vectors near UInt.max work`() throws {
        let max = UInt.max
    ...
    @Test
    func `INVARIANT: maximum possible vector 0 to UInt.max`() throws {
        // The largest possible vector
        let vector = Vector(0..<UInt.max) { $0 }

        #expect(vector.count == Vector<UInt>.Index.Count(UInt.max))

        let distance = try vector.start.position.distance.forward(to: vector.end.position)
